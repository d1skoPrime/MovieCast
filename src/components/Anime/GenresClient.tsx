'use client'
import { Anime, animeGenresDict, Genre } from '@/types/types'
import { useRef, useState } from 'react'

type Props = { genres: Genre[] }

const GenresClient = ({ genres }: Props) => {
	...
	const genresWithImages = genres.map(genre => ({
		...genre,
		picture: animeGenresDict[genre.mal_id]?.picture || '/placeholder.png'
	}))
		...
		<section className="w-full py-8">
			<div className="flex items-center justify-between mb-6 px-4">
				<div className="flex items-center gap-3">
					<h2 className="text-xl font-bold text-wite">All Genres</h2>
			...
			<div className="relative group/scroll px-4">
				...
				>
					{genresWithImages.map(item => (
						<div
							key={item.mal_id}
							onClick={() =>
								(window.location.href = `/anime/genres/${item.mal_id}`)
							}
								...
								<div className="relative h-56 sm:h-64 overflow-hidden">
									{
										<img
											src={item.picture}
											alt={item.name}
											className="w-full h-full object-cover text-center text-black flex bg-gray-900/50"
										/>
									}
									{/* Gradient Overlay */}
